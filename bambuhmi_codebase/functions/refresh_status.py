...
def refresh_status(self):
    state = APIClient.get_state(f'sensor.x1c_{PRINTER_ID}_print_status')
    if state:
        self.status_label.setText(f"Status: {state}")
    else:
        self.status_label.setText("Status: Error")
    
    error_state = APIClient.get_state(f'binary_sensor.x1c_{PRINTER_ID}_hms_errors')
    if error_state == 'on':
    ...
    remaining_time = APIClient.get_state(f'sensor.x1c_{PRINTER_ID}_remaining_time')
    self.remainingTimeLabel.setText(f"Remaining Time: {remaining_time}")

    gcode_filename = APIClient.get_state(f'sensor.x1c_{PRINTER_ID}_task_name')
    if gcode_filename:
